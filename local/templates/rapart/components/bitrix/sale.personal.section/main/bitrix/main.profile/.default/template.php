...
<div class="form__block">
		...
		<p class="main-profile-form-buttons-block col-sm-9 col-md-offset-3">
			<button type="submit" name="save" class="btn btn-themes btn-default btn-md main-profile-submit" value="<?=(($arResult["ID"]>0) ? Loc::getMessage("MAIN_SAVE") : Loc::getMessage("MAIN_ADD"))?>"><?=(($arResult["ID"]>0) ? Loc::getMessage("MAIN_SAVE") : Loc::getMessage("MAIN_ADD"))?></button>
			&nbsp;
			<button type="submit" class="btn btn-themes btn-default btn-md"  name="reset" value="<?echo GetMessage("MAIN_RESET")?>"><?echo GetMessage("MAIN_RESET")?></button>
		</p>

	</form>
	<div class="col-sm-12 main-profile-social-block">
		<?/*связать свой профиль с профилями в социальных сетях
