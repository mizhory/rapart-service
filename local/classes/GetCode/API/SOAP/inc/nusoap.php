...
class nusoap_base {
    ...
    function debug($string) {
        if ($this->debugLevel > 0) {
            $this->appendDebug($this->getmicrotime().' '.get_class($this).": $string<BR>\n");
        }
    }
    ...
    function serializeEnvelope($body, $headers = false, $namespaces = array(), $style = 'rpc', $use = 'encoded', $encodingStyle = 'http://schemas.xmlsoap.org/soap/encoding/') {
        ...
        $this->debug("In serializeEnvelope length=".StrLenByte($body)." body (max 1000 characters)=".substr($body, 0, 1000)." style=$style use=$use encodingStyle=$encodingStyle");
        $this->debug("headers:");
...
function iso8601_to_timestamp($datestr) {
    ...
    if (preg_match($pattern, $datestr, $regs)) {
        ...
        if ($regs[8] != 'Z') {
            ...
            $h = substr($regs[8], 1, 2);
            $m = substr($regs[8], StrLenByte($regs[8]) - 2, 2);
            if ($op == '-') {
                $regs[4] = $regs[4] + $h;
                $regs[5] = $regs[5] + $m;
            }
...
class soap_transport_http extends nusoap_base {
    ...
    function send($data, $timeout = 0, $response_timeout = 30, $cookies = null) {
        $this->debug('entered send() with data of length: '.StrLenByte($data));
        $this->tryagain = true;
        $tries = 0;
        while ($this->tryagain) {
            $this->tryagain = false;
    ...
    function isSkippableCurlHeader(&$data) {
        ...
        foreach ($skipHeaders as $hd) {
            $prefix = substr($data, 0, StrLenByte($hd));
            if ($prefix == $hd)
                return true;
        }
        return false;
    ...
    function decodeChunked($buffer, $lb) {
        ...
        $chunkstart = $chunkend + StrLenByte($lb);
        // while (chunk-size > 0) {
        while ($chunk_size > 0) {
            ...
            $chunkstart = $chunkend + StrLenByte($lb);
            $chunkend = strpos($buffer, $lb, $chunkstart) + StrLenByte($lb);
            if ($chunkend == false) {
    ...
    function buildPayload($data, $cookie_str = '') {
        ...
        if ($this->request_method != 'GET') {
            $this->setHeader('Content-Length', StrLenByte($data));
        }
    ...
    function sendRequest($data, $cookies = null) {
        ...
        if ($this->io_method() == 'socket') {
            // send payload
            if (!fputs($this->fp, $this->outgoing_payload, StrLenByte($this->outgoing_payload))) {
                $this->setError('couldn\'t write message data to socket');
                $this->debug('couldn\'t write message data to socket');
                return false;
            }
            $this->debug('wrote data to socket, length = '.StrLenByte($this->outgoing_payload));
            return true;
        }
        else if ($this->io_method() == 'curl') {
            // set payload
        ...
        if ($this->io_method() == 'socket') {
            ...
            while (!isset($lb)) {
                ...
                if (feof($this->fp)) {
                    ...
                    $this->debug('found no headers before EOF after length '.StrLenByte($data));
                    $this->debug("received before EOF:\n".$data);
                    $this->setError('server failed to send headers');
                    return false;
                }
                $tmp = fgets($this->fp, 256);
                $tmplen = StrLenByte($tmp);
                $this->debug("read line of $tmplen bytes: ".trim($tmp));
                if ($tmplen == 0) {
                    $this->incoming_payload = $data;
                    $this->debug('socket read of headers timed out after length '.StrLenByte($data));
            ...
            // store header data
            $this->incoming_payload .= $data;
            $this->debug('found end of headers after length '.StrLenByte($data));
            // process headers
            ...
            do {
                if ($chunked) {
                    ...
                    if ($tmplen == 0) {
                        $this->incoming_payload = $data;
                        $this->debug('socket read of chunk length timed out after length '.StrLenByte($data));
                ...
                $StrLenByte = 0;
                while (($StrLenByte < $content_length) && (!feof($this->fp))) {
                    $readlen = min(8192, $content_length - $StrLenByte);
                    $tmp = fread($this->fp, $readlen);
                    $tmplen = StrLenByte($tmp);
                    $this->debug("read buffer of $tmplen bytes");
                    if (($tmplen == 0) && (!feof($this->fp))) {
                        $this->incoming_payload = $data;
                        $this->debug('socket read of body timed out after length '.StrLenByte($data));
                        $this->debug("read before timeout:\n".$data);
                ...
                if ($chunked && ($content_length > 0)) {
                    ...
                    if ($tmplen == 0) {
                        $this->incoming_payload = $data;
                        $this->debug('socket read of chunk terminator timed out after length '.StrLenByte($data));
                        $this->debug("read before timeout:\n".$data);
            ...
            $this->debug('read body of length '.StrLenByte($data));
            $this->incoming_payload .= $data;
            $this->debug('received a total of '.StrLenByte($this->incoming_payload).' bytes of data from server');
            // close filepointer
            if (
            ...
            $this->debug('cleaned data, stringlen: '.StrLenByte($data));
            // clean headers
            foreach ($header_array as $header_line) {
                $arr = explode(':', $header_line, 2);
                if (count($arr) > 1) {
        ...
        if (isset($this->incoming_headers['content-encoding']) && $this->incoming_headers['content-encoding'] != '') {
            if (strtolower($this->incoming_headers['content-encoding']) == 'deflate' || strtolower($this->incoming_headers['content-encoding']) == 'gzip') {
                ...
                if (function_exists('gzinflate')) {
                    ...
                    $datalen = StrLenByte($data);
                    if ($this->incoming_headers['content-encoding'] == 'deflate') {
                        if ($degzdata = @gzinflate($data)) {
                            $data = $degzdata;
                            $this->debug('The payload has been inflated to '.StrLenByte($data).' bytes');
                            if (StrLenByte($data) < $datalen) {
                                // test for the case that the payload has been compressed twice
                                $this->debug('The inflated payload is smaller than the gzipped one; try again');
                                if ($degzdata = @gzinflate($data)) {
                                    $data = $degzdata;
                                    $this->debug('The payload has been inflated again to '.StrLenByte($data).' bytes');
                                }
                            }
                        }
                        else {
                    ...
                    elseif ($this->incoming_headers['content-encoding'] == 'gzip') {
                        if ($degzdata = @gzinflate(substr($data, 10))) { // do our best
                            $data = $degzdata;
                            $this->debug('The payload has been un-gzipped to '.StrLenByte($data).' bytes');
                            if (StrLenByte($data) < $datalen) {
                                // test for the case that the payload has been compressed twice
                                $this->debug('The un-gzipped payload is smaller than the gzipped one; try again');
                                if ($degzdata = @gzinflate(substr($data, 10))) {
                                    $data = $degzdata;
                                    $this->debug('The payload has been un-gzipped again to '.StrLenByte($data).' bytes');
                                }
                            }
                        }
                        else {
    ...
    function parseCookie($cookie_str) {
        ...
        if ($start > 0) {
            $domain = substr($cookie_str, $start + StrLenByte($cookie_param));
        ...
        $start = strpos($cookie_str, $cookie_param);
        if ($start > 0) {
            $expires = substr($cookie_str, $start + StrLenByte($cookie_param));
        ...
        $start = strpos($cookie_str, $cookie_param);
        if ($start > 0) {
            $path = substr($cookie_str, $start + StrLenByte($cookie_param));
...
class nusoap_server extends nusoap_base {
    ...
    function service($data) {
        ...
        $this->debug("In service, request method=$rm query string=$qs StrLenByte(\$data)=".StrLenByte($data));
        if ($rm == 'POST') {
    ...
    function invoke_method() {
        ...
        $class = '';
        $method = '';
        if (StrLenByte($delim) > 0 && substr_count($this->methodname, $delim) == 1) {
            $try_class = substr($this->methodname, 0, strpos($this->methodname, $delim));
            if (class_exists($try_class)) {
                // get the class and method name
                $class = $try_class;
                $method = substr($this->methodname, strpos($this->methodname, $delim) + StrLenByte($delim));
    ...
    function send_response() {
        ...
        // this data.
        //		if (StrLenByte($payload) > 1024 && isset($this->headers) && isset($this->headers['accept-encoding'])) {
        ...
        //		}
        //end code
        $this->outgoing_headers[] = "Content-Length: ".StrLenByte($payload);
    ...
    function parseRequest($headers, $data) {
        $this->debug('Entering parseRequest() for data of length '.StrLenByte($data).' headers:');
        $this->appendDebug($this->varDump($headers));
            ...
            foreach ($this->bindings as $bindingName => $attrs) {
                ...
                foreach ($attrs['operations'] as $opName => $opParts) {
                    ...
                    if (isset($opParts['documentation']) && $opParts['documentation'] != '') {
                        $portType_xml .= "\n".'    <documentation>'.htmlspecialcharsbx($opParts['documentation']).'</documentation>';
                    }
...
class nusoap_parser extends nusoap_base {
    ...
    function __construct($xml, $encoding = 'UTF-8', $method = '', $decode_utf8 = true) {
        ...
        if (!empty($xml)) {
            ...
            $this->debug('Entering nusoap_parser(), length='.StrLenByte($xml).', encoding='.$encoding);
    ...
    function start_element($parser, $name, $attrs) {
        ...
        // set name
        $this->message[$pos]['name'] = htmlspecialcharsbx($name);
        // set attrs
...
class nusoap_client extends nusoap_base {
    ...
    function call($operation, $params = array(), $namespace = 'http://tempuri.org', $soapAction = '', $headers = false, $rpcParams = null, $style = 'rpc', $use = 'encoded') {
        ...
        if ($this->debug) {
            ...
            $this->debug('SOAP message length='.StrLenByte($soapmsg).' contents (max 1000 bytes)='.substr($soapmsg, 0, 1000));
        }
        // send
        if (isset($this->requestHeaders) && isset($this->requestHeaders['accept-encoding'])) {
            if (strstr($this->requestHeaders['accept-encoding'], 'gzip')) {
                if (function_exists('gzencode')) {
                    if (isset($this->debug_flag) && $this->debug_flag) {
                        $soapmsg .= "<!-- Content being gzipped -->";
                    }
                    $this->http_encoding = 'gzip';
                    $soapmsg = gzencode($soapmsg);
                }
                else {
                    if (isset($this->debug_flag) && $this->debug_flag) {
                        $soapmsg .= "<!-- Content will not be gzipped: no gzencode -->";
                    }
                }
            }
            elseif (strstr($this->requestHeaders['accept-encoding'], 'deflate')) {
                // Note: MSIE requires gzdeflate output (no Zlib header and checksum),
                // instead of gzcompress output,
                // which conflicts with HTTP 1.1 spec (http://www.w3.org/Protocols/rfc2616/rfc2616-sec3.html#sec3.5)
                if (function_exists('gzdeflate')) {
                    if (isset($this->debug_flag) && $this->debug_flag) {
                        $soapmsg .= "<!-- Content being deflated -->";
                    }
                    $this->http_encoding = 'deflate';
                    $soapmsg = gzdeflate($soapmsg);
                }
                else {
                    if (isset($this->debug_flag) && $this->debug_flag) {
                        $soapmsg .= "<!-- Content will not be deflated: no gzcompress -->";
                    }
                }
            }
        }
    ...
    function send($msg, $soapaction = '', $timeout = 0, $response_timeout = 30) {
        if ($debug['ServiceCall']) {
            $logData = simplexml_load_string($msg);
            if (function_exists('dom_import_simplexml')) {
                $dom = dom_import_simplexml($logData)->ownerDocument;
                $dom->formatOutput = true;
                SMGeneralLog::AddFileLogRecords(array('VALUE' => $dom->saveXML(), 'CATEGORY' => 'NUSOAP_MESSAGE_OUT','DEPTH_LEVEL'=>2));
            }
            else
                SMGeneralLog::AddFileLogRecords(array('VALUE' => $msg, 'CATEGORY' => 'NUSOAP_MESSAGE_OUT','DEPTH_LEVEL'=>2));
        }
        $this->checkCookies();
        // detect transport
        switch (true) {
            ...
            case preg_match('/^http/', $this->endpoint):
                ...
                if ($this->http_encoding != '') {
                    ...
                    $http->setHeader('Content-Encoding', $this->http_encoding);
                }
                $this->debug('sending message, length='.StrLenByte($msg));
                ...
                elseif ($this->getError()) {
                    ...
                    $this->debug('got response, length='.StrLenByte($this->responseData).' type='.$http->incoming_headers['content-type']);
    ...
    function parseResponse($headers, $data) {
        ...
        $DB->DoConnect();
        if ($debug['ServiceCall']) {
            $logData = simplexml_load_string($data);
            if (function_exists('dom_import_simplexml')) {
                $dom = dom_import_simplexml($logData)->ownerDocument;
                $dom->formatOutput = true;
                if (method_exists($dom,'saveXML'))
                    SMGeneralLog::AddFileLogRecords(array('VALUE'=>unicon($dom->saveXML(),false),'CATEGORY'=>'NUSOAP_MESSAGE_IN','DEPTH_LEVEL'=>2));
                else
                    SMGeneralLog::AddFileLogRecords(array('VALUE'=>unicon($data,false),'CATEGORY'=>'NUSOAP_MESSAGE_IN','DEPTH_LEVEL'=>2));
            }
            else
                SMGeneralLog::AddFileLogRecords(array('VALUE' => unicon($data, false), 'CATEGORY' => 'NUSOAP_MESSAGE_IN','DEPTH_LEVEL'=>2));
        }
        if ($this->debug) {
            $this->debug('Entering parseResponse() for data of length '.StrLenByte($data).' headers:');
            $this->appendDebug($this->varDump($headers));
        }
        if (!isset($headers['content-type'])) {
        ...
        foreach ($this->operations as $operation => $opData) {
            if ($operation != '') {
                ...
                if (sizeof($opData['input']['parts']) > 0) {
                    ...
                    $paramStr = substr($paramStr, 0, StrLenByte($paramStr) - 2);
                    $paramArrayStr = substr($paramArrayStr, 0, StrLenByte($paramArrayStr) - 2);
                    $paramCommentStr = substr($paramCommentStr, 0, StrLenByte($paramCommentStr) - 2);
                }
                else {
                    $paramStr = '';
                    $paramArrayStr = '';
